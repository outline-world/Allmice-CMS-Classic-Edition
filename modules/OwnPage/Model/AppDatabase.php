...
class AppDatabase extends DatabaseCms
{
	...
	public function getConfigData($whereClauseEnd)
	{
		...
		$sqlString.=" WHERE module_name = 'OwnPage'";
		if($whereClauseEnd!="")
	...
	public function getPageDetails($id)
	{
		...
		$resultSet = $stmt->fetchAll(PDO::FETCH_ASSOC);
		foreach ($resultSet as $row) {
			$row=$this->decodeDbRow($row,array("title","description","body"));
			$pageDetails = $row;
		}
	...
	public function getPostDetails($id)
	{
		...
		$postDetails=array();
		$sqlString="SELECT id, creator_id, content, status";
	...
	public function savePage($page,$uri,$menuItem,$config)
	{
		...
		if($aliasExists<1 && $pathExists<1){
			...
			if ($id > 0) {
				...
				try {
					...
					if($config['roleAccessSwitch']=="on" && $config['botTagSwitch']=="on"){
	...
	public function getPostShortDetails($id)
	{
		...
		$sqlString="SELECT id, creator_name, creator_id, time_created, SUBSTR(content,1,100) AS content";
		$sqlString.=" FROM ".$this->tablePrefix."mod_page_post";
		$sqlString.=" WHERE id = :id";
