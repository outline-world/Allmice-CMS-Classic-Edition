...
class PageForm extends Form
{
	...
	public function __construct()
	{
		...
		$this->add(array(
			...
			'guide' => 'Choose, for which roles this page will be cached.',
		));
		...
		$this->add(array(
			...
			'guide' => 'Choose, which roles can view this page.',
		));
	...
	public function getDefaultRoles($config, $roleSet)
	{

		$tempArr=explode(", ", $config['addPageEvent']['defaultRoleAccess']);

		$this->accessFieldStatus=$config['addPageEvent']['roleAccessSwitch'];

		$defaultRoleValues=array();
		for($i=0;$i<count($roleSet);$i++){
			if(in_array($roleSet[$i]['title'],$tempArr)){
				$defaultRoleValues[]=$roleSet[$i]['id'];
			}
		}
	...
	public function getDefaultCacheRoles($config, $roleSet)
	{

		$tempArr=explode(", ", $config['addPageEvent']['defaultRoleCache']);

		$this->cacheFieldStatus=$config['addPageEvent']['roleCacheSwitch'];
